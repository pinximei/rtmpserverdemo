...
- 自实现 RTMP 1.0 server（监听 1935，编码出帧后直接 push 到 server 的发布通道）
...
- NetStream：publish 入口（直接由本地 pipeline 注入，不走外部 publish 客户端）/ play（拉流端）/ deleteStream / closeStream
...
├── RtmpServer.{h,cpp}        监听 1935 + ClientSession + 异步 sender + 本地 publish 通道
